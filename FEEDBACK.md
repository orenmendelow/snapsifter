...
- [ ] **S21-4**: PICK PHOTOS modal shows a broken grid — photos are squished/cropped into horizontal slices. Rendering bug in the photo picker grid.
- [ ] **S21-5**: Collage photos shot with a different recipe setting need a small icon in the corner. Hover shows which setting differs from the majority.
...
- [ ] **S21-7**: Up/Down arrow key functions are flipped — need to reverse them.
- [ ] **S21-8**: Left/Right arrow transition between collage and focus should animate — slide the collage away and center the photo, not a jarring snap.
